...
- **输入支持**：
  ...
  - Markdown 编辑器（可二次编辑）
  - Word(.docx) 文件上传（客户端 mammoth 解析为 Markdown；**文字先出、图片异步补**——解析时文字立即渲染，图片先以本地预览显示、后台并发上传图床后再替换为真实链接，避免多图 Word 卡顿 / 误以为解析失败）
  - AI 文案生成（输入提示词让模型帮你写）
...
- **稳定锚点（均已打 git tag，可随时回退）**：
  - `v20260805.033`（commit `d1187bb`）：当前主版本。Word 上传「文字先出 + 图片异步补」、模型下拉 LOGO 头像、模型管理「保存并使用」/ 自动选中 / 二次确认、4 套界面主题色、5 个预设模型（Agnes/DeepSeek/Kimi/智谱/Qwen）。
    ```bash
    git checkout v20260805.033   # 检出后重新构建部署即可
    ```
