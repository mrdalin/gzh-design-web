...
- **稳定锚点（均已打 git tag，可随时回退）**：
  - `v20260805.037`（commit `1519cf0`）：**当前推荐版本**。Word 图片上传体验再打磨：无 Key 时富文本灰块与 Markdown 占位符共用清晰文案，未配置图片 API 时 Markdown 区绝不会泄漏 base64；配置了 Key 后上传图片会弹出全屏顶层进度遮罩（已完成 X / 共 Y 张），防止用户干等或误编辑。
    ```bash
    git checkout v20260805.037   # 检出后重新构建部署即可
    ```
  - `v20260805.036`（commit `a4f42bb`）：v035 的强化版。把 Word 图片上传打磨到「稳定好用」：解析进度改为受控内联状态条（告别常驻不消失的 Toast）、Markdown 区彻底杜绝 base64 残留（未上传完或失败一律整洁占位）、imgbb 出错时给出人话版具体原因；上传链路也更稳更快。可作为 v037 之前的兜底。
