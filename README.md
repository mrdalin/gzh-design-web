...
- **多模型接入（BYOK）**：内置 Agnes-CN / DeepSeek / Kimi 三个预设，并支持添加任意多个自定义 OpenAI 兼容模型。API Key 由用户自己填写（存浏览器 localStorage），不落服务端。
- **6 套内置文章主题**：摸鱼绿 / 红白色系 / 石墨极简 / 留白禅意 / 摸鱼票据 / 橄榄手记，鼠标悬停可预览。
- **自定义主题生成**：按描述或参考图生成一套新主题组件库并本地保存复用。
- **界面主题色切换**：右上角三个小色块，一键切换全站主色——Semi 蓝 / 公众号绿 / 极简白灰，选择写入 `localStorage` 刷新不丢。
- **模型下拉显示实际模型名**：下拉列表与选中态直接显示模型标识（如 `agnes-2.5-flash`），而非渠道名，一眼看清当前用的是哪个模型。
- **输入支持**：
  - 富文本粘贴（从 Word / 网页直接带格式粘贴）
  - Markdown 编辑器（可二次编辑）
  - Word(.docx) 文件上传（客户端解析为 Markdown，配图可选自动上传图床）
  - AI 文案生成（输入提示词让模型帮你写）
...
├── colorThemes.ts      界面主题色预设（蓝/绿/白灰）+ 运行时注入逻辑
├── lib/
...
| 维度 | 文章主题（6 套） | 界面主题色（3 套） |
|------|----------------|------------------|
| 作用对象 | 文章排版样式（标题/正文/卡片/配色方案） | 全站 UI 主色（按钮/链接/边框/选中态） |
| 选择位置 | 「选择主题」一行的卡片 | 右上角三个小色块 |
| 选项 | 摸鱼绿、红白色系、石墨极简、留白禅意、摸鱼票据、橄榄手记 | Semi 蓝 `#3491FA`、公众号绿 `#069A4C`、极简白灰 `#555555` |
| 持久化 | 当前选择随内容一起保存 | `localStorage('gzh-theme-color')`，默认公众号绿 |

> 网站 logo 与浏览器 favicon 固定为公众号绿品牌色，不受界面主题色切换影响（有意保留的品牌锚点）。
...
- **稳定锚点**：`v20260805.026`（commit `d6eb949`）已打 git tag。如需回退到该版本：
  ```bash
  git checkout v20260805.026   # 检出后重新构建部署即可
  ```
- 完整变更历程见下方「更新历程」。
