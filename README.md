...
- **输入支持**：
  ...
  - AI 文案生成（输入提示词让模型帮你写）
- **图床（BYOK）**：接入 imgbb，自己填 key，用于排版中插图上传。上传通道已**免 base64**（浏览器直接以二进制字节发往服务端，再由服务端包成 multipart 转发 imgbb）；并对 imgbb 的错误码做了中文细分提示（限流 / Key 无效 / 超 32MB / 格式损坏 / 维护页等），所有上传入口（文件、富文本插图、Word）的错误提示更可读、可操作。
...
### 更新历程（v006 → v20260806.005）

| 版本 | 日期 | 关键改动 |
|------|------|---------|
| v006 | 2026-08-04 | 补全 favicon（svg + 多尺寸 ico） 
| v007 | 2026-08-04 | 导出文件名改为短格式（标题前3字 + 时间戳） 
| v008 | 2026-08-04 | 导出文件名加入主题名 
| v009 | 2026-08-04 | 时间戳去秒 + 页面 logo 改为与 favicon 同款绿色 SVG 
| v010 | 2026-08-04 | 主题栏加「选择主题」提示 
| v011 | 2026-08-04 | 提示精简为 4 字 + 修复悬停预览图被裁剪不显示 
| v012 | 2026-08-04 | 预览图按主题 id 匹配，修复 4 个主题不显示 
| v013 | 2026-08-04 | 主题展示顺序调整 + 方图/竖长图样式区分 
| v014 | 2026-08-04 | 提示「选择主题」竖排绿字 
| v015 | 2026-08-04 | 编辑框 U 型边框修复 + 全站主色统一为公众号绿（重要：Semi 变量机制） 
| v016 | 2026-08-04 | 主色由 `#07C160` 调为更沉稳的 `#069A4C` 
| v017 | 2026-08-05 | 未配置模型 Key 提示文案修正 
| v018 | 2026-08-05 | 右上角 3 主题色切换器（蓝/绿/白灰，localStorage 持久化） 
| v019 | 2026-08-05 | 色块移至「选择主题」行最右 + 版本号回归网站名后 
| v020 | 2026-08-05 | 色块移回 header、模型下拉前；保持 header 高度 
| v021 | 2026-08-05 | 顶部控件间距统一为 12px 
| v022 | 2026-08-05 | 排版历史数字角标（99+）+ 模型管理提示语修正 + Agnes 标注「免费开通 API」 
| v023 | 2026-08-05 | 角标 0 条不显示 + 字号缩小 + 提示语去掉多余「经」字 
| v024 | 2026-08-05 | API Key 空值红框提示 
| v025 | 2026-08-05 | 必填项（API地址/API KEY/模型名称）红框一致性修复 
| v026 | 2026-08-05 | 模型下拉显示实际模型名 + Header 按钮加卡片风格圆角边框 
| v027 | 2026-08-05 | logo/favicon 跟随界面主题色 + 新增公众号橙红第 4 套界面色（红蓝绿黑排序） 
| v028 | 2026-08-05 | 模型列表 / 弹窗加入品牌 LOGO 头像；Header 模型下拉改 Dropdown+Button（勾选图标用 IconTick 避坑） 
| v029 | 2026-08-05 | 头像统一用显示名首字母；新增「智谱 GLM(Z)」「通义千问(Q)」两个预设 
| v030 | 2026-08-05 | 模型管理保存后若无可用模型则自动设为「使用中」 
| v031 | 2026-08-05 | 「恢复默认模型」按钮加 Popconfirm 二次确认，防误点丢失 API Key 
| v032 | 2026-08-05 | 模型管理新增「保存并使用」按钮（强制切换到刚保存的模型） 
| v033 | 2026-08-05 | Word 上传解析改为「文字先出 + 图片异步补」（方案 A+D）；修复终态 Markdown 未回写实 URL 的问题 
| v034 | 2026-08-05 | Word 上传时 Markdown 区改用「图片占位符 + 待更新」替代冗长 base64；图片后台上传期间「生成排版」按钮自动禁用，避免内容未就绪就被提交 
| v035 | 2026-08-05 | 修复 v034 回归：解析进度提示改为受控内联状态条（彻底解决常驻不消失）；`finish()` 回写前统一清洗，任何残留 data:image 都转为整洁占位符，永不出现 base64 长串 
| v036 | 2026-08-05 | 增强 Word 图片上传错误诊断：客户端展示首条失败的具体原因（如「imgbb 服务异常 HTTP 503」），Cloudflare 侧识别 imgbb 维护页（非 JSON 响应）并给出明确提示；上传链路稳定性与速度提升 
| v037 | 2026-08-05 | Word 图片上传体验再打磨：占位文案统一为「图片占位。请在右上角配置「图片 API」后重传 Word 以自动上传图片」；无 Key 时 Markdown 区绝不再泄漏 base64；配置了 Key 后上传图片弹出全屏顶层进度遮罩（已完成 X / 共 Y 张），防止用户干等或误编辑 
| v038 | 2026-08-05 | 顶栏体验升级：图片 API / 模型 API 未配置时显示红色未读式角标（配置后自动消失）；右上角「源码」改为 GitHub 同款「Star」风格按钮（Octicons 星形 + Star 文案 + 实时 Star 数，点击直达仓库） 
| v039 | 2026-08-05 | 修复 Word 图片占位文字截断与 Markdown base64 泄漏：占位 SVG 改为 100% 宽度 + 三行文字；富文本 → Markdown 全路径（手动转换、自动同步、粘贴/插入图片）均先清洗 data:image 再转 Markdown，中间编辑器绝不再出现 base64 长串；imgbb 上传附带 `name` 参数，便于图床管理文件名 
| v20260806.001 | 2026-08-06 | 替换被误删的 6 张主题预览图为 imgbb 新地址（纯资源地址修正，非功能调整） 
| v20260806.002 | 2026-08-06 | imgbb 图床优化：上传通道免 base64（浏览器以二进制字节直传、服务端再包 multipart 转发 imgbb，省内存省带宽）；服务端对 imgbb 错误码做中文细分提示（限流 429 / Key 无效 / 超 32MB / 格式损坏 / 维护页），所有上传入口的错误更可读可操作 
| v20260806.003 | 2026-08-06 | 尝试把 CF→imgbb 改回 base64 字符串以绕开 502，但线上仍被 Cloudflare 平台层拦截（回归失败，仅记录） 
| v20260806.004 | 2026-08-06 | **图片上传改浏览器直连 imgbb**，彻底解决 CF 代理 `/api/upload` 必 502：浏览器直接 POST 原字节 FormData 到 `https://api.imgbb.com/1/upload`（免 base64，BYOK key 本就在浏览器）；`data.data.url` 直链解析 + 错误码中文细分前置到前端；`functions/api/upload.ts` 标记 DEPRECATED 
| v20260806.005 | 2026-08-06 | 多图 Word 上传限流防护：Word 图片上传改**并发受限队列**（最多 3 张同时）避免瞬时打满 imgbb 免费额度（约 30 张/分钟）；`uploadImageBytes` 增加 429/网络抖动**指数退避重试**（最多 4 次，仅限流重试、Key/体积/格式错误不重试），几十张图也能稳定传完 
