...
- **稳定锚点（均已打 git tag，可随时回退）**：
  - `v20260805.038`（commit `5eb6dbc`）：**当前推荐版本**。顶栏交互再升级：图片 API / 模型 API 在未配置时显示红色未读式角标，点进去配好即自动消失，再也不怕「忘了配 Key」；右上角原「源码」入口升级为 GitHub 官方同款「Star」风格按钮（Octicons 星形图标 + Star 文案 + 实时 Star 数，点击直达仓库）。
    ```bash
    git checkout v20260805.038   # 检出后重新构建部署即可
    ```
  - `v20260805.037`（commit `1519cf0`）：上一个稳定版。Word 图片上传体验打磨：无 Key 时 Markdown 绝不再泄漏 base64，配置 Key 后上传弹出全屏进度遮罩。可作 v038 之前的兜底。
