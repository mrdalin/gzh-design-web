...
- **稳定锚点（均已打 git tag，可随时回退）**：
  - `v20260806.004`（commit `9672e09`）：**上传可用基线（仍推荐）**。图片上传改为**浏览器直连 imgbb**（`https://api.imgbb.com/1/upload`，原字节 FormData、免 base64），彻底解决之前经 Cloudflare Functions 代理 `/api/upload` 上传必 502 的问题（CF 出口 fetch 到 api.imgbb.com 被平台层整体拦截，与代码无关）。**这是首个能正常上传图片的版本**（v005 起在其基础上增强多图限流，上传持续可用）；imgbb 错误码中文细分提示也随之前置到前端。
    ...
    ```
  - `v20260812.011`（commit `54188e0`，**当前最新功能版**，版本号见 `src/config.ts` 的 `APP_VERSION`）：自定义主题向导切换按钮组增加上边距（与下边距一致 16px），不再紧贴上方说明小字。
  - `v20260812.010`（commit `68b241b`）：自定义主题向导修复——「参考图生成」提示语文案更新、切换按钮选中态背景撑满整个 Tab 格宽并修复 hover 变白、模型下拉改受控展开（点未配置模型先收下拉再开管理弹窗，不再遮挡）。
  - `v20260812.009`（commit `9ccbb6f`）：自定义主题向导优化——切换按钮大字号+图标+平分宽度选中态撑满、主题名称与生成模型同行各半、模型下拉「识图」标记、未配置模型点击打开模型管理（onManageModels 回调，不改顶部全局模型）。
  - `v20260812.008`（commit `912db64`）：界面细节优化——排版历史弹窗加小字说明、主题栏删「+ 自定义主题」按钮（卡片统一进向导）、Star 按钮暂隐藏（代码保留）、预设模型通义千问与智谱 GLM 顺序互换、「支持图片」勾选移到模型名称下方。
  - `v20260812.007`（commit `51a17e8`）：自定义主题向导两种方式改为平分宽度按钮组切换，选中态高亮填充（与「风格」行按钮样式一致），区分更清楚。
...
| v20260812.008 | **界面细节优化**：①排版历史弹窗标题下加小字说明「每次排版渲染成功后自动保存，点击预览可回看效果」；②主题栏删除末尾「+ 自定义主题」按钮，仅保留「自定义主题」卡片（点击卡片统一进入向导：未生成引导生成、已生成可修改，应用后自动选中）；③顶栏「Star」按钮暂隐藏（代码注释保留，随时可恢复）；④预设模型顺序调整——通义千问与智谱 GLM 互换（agnes → deepseek → kimi → qwen → glm）；⑤模型编辑弹窗中「支持图片」勾选移到「模型名称」输入框下方，语义更贴切 |
| v20260812.009 | **自定义主题向导优化**：①两种生成方式切换按钮改为大字号、图标（✎ 文字描述 / 🖼 上传参考图）、平分宽度且选中态背景撑满整个格子并垂直居中；②「主题名称」与「生成模型」同一行各占一半（不再紧凑挤在一起）；③模型下拉中支持图片的模型显示「识图」标记（原「视觉」）；④下拉选择未配置的模型时与顶部一致——提示并打开模型管理弹窗（通过新增 `onManageModels` 回调，不影响顶部全局选中模型） |
| v20260812.010 | **自定义主题向导修复**：①「参考图生成」提示语文案改为「需要支持识图的模型，请先在「模型管理」为支持识图的该模型勾选「支持图片」后选择该模型」；②两种生成方式切换按钮的选中态背景改为撑满整个 Tab 格宽（选中背景移到带 flex:1 的 label 上，不再只包住图标+文字宽度），并修复已选中 Tab 上 hover 时背景变白导致文字看不清的问题；③模型下拉改为受控展开——点击未配置模型或「管理模型」时先收起下拉再打开模型管理弹窗，不再遮挡 |
| v20260812.011 | 自定义主题向导「文字描述 / 上传参考图」切换按钮组增加上边距（与下边距一致 16px），不再紧贴上方说明小字 |
